...
from data import ler_arquivo, gerar_ranking, realizar_login, verificar_usuario, obter_estatisticas_gerais
...
#texto centralizado na posição X, Y
...
def desenhar_menu(tela, posicao_mouse):
    ...
    if agora - ultimo_update > intervalo:
        tamanho_titulo += direcao_titulo * 0.2  # Velocidade do pulso
    ...
    #título com efeito de sombra e animação
    ...
    #BOTÕES
    lista_botoes = ["Jogar", "Instruções", "Pontuação", "Sair"]
    inicio_y = 180
    espacamento = 70
...
def desenhar_instrucoes(tela, posicao_mouse):
    ...
    regras = [
        "REGRAS PRINCIPAIS:",
        "1. Primeira jogada (X): pode jogar em qualquer célula de qualquer mini tabuleiro.",
        "2. Onde você joga define onde o oponente deve jogar.",
        "3. Se o mini tabuleiro estiver cheio ou já vencido, o jogador pode jogar em qualquer",
        "outro disponível.",
        "4. Para vencer um mini tabuleiro: faça 3 símbolos em linha, coluna ou diagonal.",
        "5. Para vencer o jogo: conquiste 3 mini tabuleiros alinhados no tabuleiro maior."
    ]
    ...
    for linha in regras:
        ...
        desenhar_texto_centralizado(
            tela, linha, fonte, cor, LARGURA_TELA // 2,pos_y )

        pos_y += 35

    desenhar_texto_centralizado(tela, "Divirta-se!", fonte_pequena, COR_BOTAO, LARGURA_TELA//2,
        ALTURA_TELA - 130 )
    
    #botão de voltar
    botao_voltar = desenhar_botao(tela, "Voltar", LARGURA_TELA//2 - 100, ALTURA_TELA - 80,
        200, 50, posicao_mouse )
...
def desenhar_tabuleiro_ultimate(tela, jogo, simbolo_jogador, simbolo_ia, posicao_mouse):
    ...
    tela.fill(COR_FUNDO)
    desenhar_texto_centralizado(tela, "JOGO DA VELHA ULTIMATE", fonte_titulo, COR_TEXTO, LARGURA_TELA//2, 35)
    ...
    for linha_g in range(3):
        for col_g in range(3):
            ...
            #se o tabuleiro grande já tem dono, desenha um simbolo grande
            if tabuleiro_global[idx_grande] != '':
            ...
            #desenha os símbolos pequenos das células
            ...
            for sr in range(3):
                for sc in range(3):
                    ...
                    #Hover nas células vazias válidas
                        ...
                        if rect_celula.collidepoint(posicao_mouse) and (jogo.proximo_tabuleiro_foco is None or jogo.proximo_tabuleiro_foco == idx_grande):
                            ...
                            superficie_transparente.fill((255, 255, 255, 30))
                            tela.blit(superficie_transparente, (rect_celula.x+2, rect_celula.y+2))

    #botões de controle no jogo quando se esta jogando
    botoes = {
...
def desenhar_placar(tela, posicao_mouse):
    ...
    linhas_stats = [
        ...
        (f"Vitórias Jogador: {stats['vitorias_jogador']}", COR_JOGADOR_X),
        (f"Vitórias IA: {stats['vitorias_ia']}", COR_JOGADOR_O),
    ...
    #melhores jogadores
    desenhar_texto_centralizado(tela, "Melhores Jogadores", fonte_media, (255, 215, 0), LARGURA_TELA - 200, 150)
...
def desenhar_entrada_nome(tela, nome_atual):
    ...
    tela.fill(COR_FUNDO)
    desenhar_texto_centralizado(tela, "LOGIN", fonte_titulo, COR_JOGADOR_O, LARGURA_TELA//2, 120)
    desenhar_texto_centralizado(tela, "Digite seu Nome:", fonte_pequena, COR_TEXTO, LARGURA_TELA//2, 200)
...
def desenhar_fim_de_jogo(tela, vencedor, posicao_mouse):
    """Tela de Game Over com feedback visual."""
    tela.fill(COR_FUNDO)
    
    # 1. Título e Subtítulo baseados no resultado
    if vencedor == 'X':
        texto_titulo = "VITÓRIA!"
        texto_sub = "Você dominou a IA."
        cor_titulo = (255, 215, 0) # Dourado
        cor_vencedor = COR_JOGADOR_X
    elif vencedor == 'O':
        texto_titulo = "DERROTA..."
        texto_sub = "A IA foi mais esperta desta vez."
        cor_titulo = COR_JOGADOR_O # Azul/Ciano
        cor_vencedor = COR_JOGADOR_O
    else:
        texto_titulo = "EMPATE!"
        texto_sub = "Uma batalha extremamente equilibrada."
        cor_titulo = COR_TEXTO
        cor_vencedor = (150, 150, 150)

    # Desenha os textos
    desenhar_texto_centralizado(tela, texto_titulo, fonte_titulo, cor_titulo, LARGURA_TELA//2, 150)
    desenhar_texto_centralizado(tela, texto_sub, fonte_pequena, COR_TEXTO, LARGURA_TELA//2, 220)

    # 2. Desenha o Símbolo Gigante do Vencedor (Decorativo)
    if vencedor:
        # Usa uma fonte temporária gigante ou a fonte_titulo escalada
        fonte_gigante = pygame.font.Font(CAMINHO_FONTE, 120)
        
        # Sombra
        desenhar_texto_centralizado(tela, vencedor, fonte_gigante, (0,0,0), LARGURA_TELA//2 + 4, 320 + 4)
        # Frente
        desenhar_texto_centralizado(tela, vencedor, fonte_gigante, cor_vencedor, LARGURA_TELA//2, 320)

    # 3. Botões de Ação
    # Botão Jogar Novamente
    rect_jogar = desenhar_botao(
        tela, "Jogar Novamente", 
        LARGURA_TELA//2 - 120, ALTURA_TELA - 180, 
        265, 50, posicao_mouse
    )
    
    # Botão Menu
    rect_menu = desenhar_botao(
        tela, "Voltar ao Menu", 
        LARGURA_TELA//2 - 120, ALTURA_TELA - 110, 
        265, 50, posicao_mouse
    )

    return {"JOGAR_NOVAMENTE": rect_jogar, "IR_MENU": rect_menu}